...
//		rnd.nextInt(4); 
	...
	public void populateOne() {
		...
		//board[0][0] = 2; //example way to set an element in board
		
		// generate r and c
		// while (board[r][c] != 0 && numOpenSpaces > 0) {
  		
//	}
		//after loop, you know it's ok to use the generated r, c values
		//example way to generate numbers using the rnd object
		...
		while ( numOpenSpaces >= 0 && board[r][c] != 0 ) {
			r = rnd.nextInt(4);
			c = rnd.nextInt(4);
			// board[r][c] = 2;
		}
	...
	public void slideRight(int[] row) {
        ...
        int temp;
        for ( int total = 0; total < row.length - 1; total++) {
	...
	//	slideRight(board[0]); //call helper method to do the work
	//	slideRight(board[1]); 
	//	slideRight(board[2]); //pseudocode 
	}
	...
	public void slideUp() {
		...
		for ( int c = 0; c < board[0].length; c++ ) {
			int[] arr = new int[board[c].length];
